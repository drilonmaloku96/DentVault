<script lang="ts">
	...
	async function confirmCreate() {
		...
		try {
			...
			createError = e instanceof Error ? e.message : i18n.t.timeline.vaultDrop.createFailed;
		}
	}
	...
	async function onGripPointerUp(e: PointerEvent) {
		...
		try {
			...
			loadError = err instanceof Error ? err.message : i18n.t.timeline.vaultDrop.moveFailed;
		}
	}
	...
	async function handleSave() {
		...
		try {
			...
			loadError = e instanceof Error ? e.message : i18n.t.timeline.vaultDrop.saveFailed;
		} finally {
			isSaving = false;
		}
	}
...
<Dialog bind:open>
	<DialogContent class="max-w-md sm:max-w-md flex flex-col gap-0 p-0 overflow-hidden max-h-[80vh]">
		...
		<DialogHeader class="px-5 pt-5 pb-3 shrink-0">
			<DialogTitle class="text-sm font-semibold">{i18n.t.timeline.vaultDrop.title}</DialogTitle>
		</DialogHeader>
		...
		<div class="flex-1 overflow-y-auto min-h-0 py-2" data-folder-rel="">
			...
			{:else if tree.length === 0}
				<p class="px-5 text-xs text-muted-foreground italic">{i18n.t.timeline.vaultDrop.noFolders}</p>
			{:else}
				{#each tree as node}
		...
		>
			...
			>
				...
				{i18n.t.timeline.vaultDrop.newFolder}
			</button>
			{#if dragOverRel === ''}
				<span class="text-[11px] text-primary/70 ml-auto">{i18n.t.timeline.vaultDrop.dropHereRoot}</span>
			{/if}
		</div>

		<DialogFooter class="px-5 py-3 shrink-0 border-t border-border/50 gap-2">
			<Button variant="ghost" onclick={() => (open = false)} class="text-muted-foreground">{i18n.t.actions.cancel}</Button>
			<Button
				onclick={handleSave}
			...
			>
				{#if isSaving}
					...
					{i18n.t.timeline.vaultDrop.saving}
				{:else}
					{i18n.t.timeline.vaultDrop.saveHere}
				{/if}
			</Button>
		</DialogFooter>
	</DialogContent>
	...
	<div class="group">
		...
		>
			...
			<span
				role="button"
				tabindex="0"
				title={i18n.t.timeline.vaultDrop.newSubfolder}
...
{#snippet createInput(parentRel: string, indentPx: number = 0)}
	<div class="flex items-center gap-1.5 pr-3 py-1" style="padding-left: {12 + indentPx + 20}px;">
		...
		<input
			...
			type="text"
			placeholder={i18n.t.timeline.vaultDrop.folderNamePlaceholder}
			onkeydown={(e) => {
