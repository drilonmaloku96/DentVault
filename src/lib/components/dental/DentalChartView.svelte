<script lang="ts">
	...
	async function openExistingRestorationEditor(toothNum: number) {
		...
		const isBridge = members[0]?.condition !== 'prosthesis';
		...
		for (const m of members) {
			...
			else if (m.abutment_type === 'implant') bm.set(m.tooth_number, 'implant');
			...
			if (m.prosthesis_type) {
				pm.set(m.tooth_number, {
					...
					abutment_type: m.abutment_type === 'implant' ? 'implant' : 'tooth',
				});
			}
		}
		...
		if (result.type === 'bridge') {
			for (const tooth of restorationEditTeeth) {
				...
				await upsertToothChartEntry(patientId, tooth, {
					...
					abutment_type: isPontic ? null : isImplant ? 'implant' : 'tooth',
					prosthesis_type: null,
				});
			}
		} else {
			for (const tooth of restorationEditTeeth) {
				...
				await upsertToothChartEntry(patientId, tooth, {
					...
					abutment_type: isReplaced ? null : role.abutment_type,
...
{:else if isSnapshotReadOnly}
	<!-- ── Read-only snapshot: just the chart, no side panel ── -->
	<div class="rounded-lg border bg-card p-5 flex flex-col gap-4">
		<div class="flex items-center gap-2 rounded-md bg-muted/60 border border-border/50 px-3 py-2">
			...
			<p class="text-xs text-muted-foreground">This is a historical snapshot — read-only.</p>
		</div>
		<Separator />
		<ToothChart
			{chartData}
