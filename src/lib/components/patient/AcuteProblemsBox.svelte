<script lang="ts">
	...
	import { acuteTagOptions, type ClinicalTag } from '$lib/stores/clinicalTags.svelte';
	...
	const hashSuggestions = $derived.by(() => {
		...
		const list = acuteTagOptions.list;
		if (!q) return list.slice(0, 8);
		return list.filter(t => acuteTagOptions.displayLabel(t).toLowerCase().includes(q));
	});

	const hashShowCreate = $derived(
		hashQuery.trim().length > 0 &&
		!hashSuggestions.some(t =>
			acuteTagOptions.displayLabel(t).toLowerCase() === hashQuery.toLowerCase().trim(),
		),
	);
	...
	async function selectHashCondition(tag: ClinicalTag) {
		const label = acuteTagOptions.displayLabel(tag);
...
<div class="rounded-lg border border-red-200 dark:border-red-800 overflow-hidden">
	...
	{#if acuteTagOptions.loaded && activeTags.length > 0}
		<div class="px-4 pb-3 flex flex-wrap gap-1.5">
			{#each activeTags as tagKey (tagKey)}
				{@const tag = acuteTagOptions.list.find(t => t.key === tagKey)}
				{@const label = tag ? acuteTagOptions.displayLabel(tag) : tagKey}
				<button
					type="button"
