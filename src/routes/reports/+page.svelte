<script lang="ts">
	...
	function monthLabel(month: string): string {
		...
		return new Date(Number(year), Number(mon) - 1, 1).toLocaleString('en-GB', { month: 'short' });
	}
...
<div class="flex flex-col gap-6">
	...
	<div class="flex flex-wrap items-center gap-3 rounded-lg border bg-card p-4">
		...
		<div class="flex items-center gap-2">
			<label class="text-xs font-medium text-muted-foreground whitespace-nowrap" for="perf-doctor-select">{i18n.t.reports.performance.selectDoctor}</label>
			<select
				id="perf-doctor-select"
							...
							<div class="flex flex-col gap-1">
								...
								{#if stat.avg_planned_duration != null}
									<div class="flex flex-col gap-0.5">
										<div class="flex items-center gap-1.5">
											<span class="text-[9px] text-muted-foreground/70 w-12 text-right">{i18n.t.reports.performance.planned}</span>
										...
										{#if stat.avg_actual_duration != null}
											<div class="flex items-center gap-1.5">
												<span class="text-[9px] text-muted-foreground/70 w-12 text-right">{i18n.t.reports.performance.actual}</span>
												<div class="flex-1 h-2.5 rounded bg-muted/40 overflow-hidden">
													<div
