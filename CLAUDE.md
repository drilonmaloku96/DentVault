...
- **Sticky header stacking context trap**: `sticky` + any `z-*` creates a new CSS stacking context. Dropdowns that are `absolute` inside the header are confined to that z-level. **Never use a `fixed` backdrop inside the sticky header** — it will appear above the dropdown and block it. Use a document-level click listener (`document.addEventListener('click', ...)`) added/removed via `$effect` instead.
- **Patient header — appointments dropdown**: pill button shows next upcoming appointment; clicking opens a two-section dropdown: "Upcoming" (future scheduled, ASC) + "Previous" (past/completed/cancelled, DESC). Close logic: `$effect` adds a document `click` listener when `apptDropdownOpen` is true; removed on cleanup. `bind:this={pillWrapperEl}` on the wrapper div is used to detect outside clicks.
...
- **Timeline entry bar — Enter key submits**: plain Enter in description `contenteditable` submits. Shift+Enter inserts newline. Guard: `e.key === 'Enter' && !e.shiftKey && !e.metaKey && !e.ctrlKey`.
- **Audit log — wired operations**: `appendAuditRecord` (from `$lib/services/audit`) is called in `db.ts` for: `updatePatient`, `deletePatient`, `updateTimelineEntry`, `deleteTimelineEntry`, `upsertToothChartEntry` (updates only, skips inserts, skips no-op diffs), `deleteTreatmentPlan`, `deleteDocument`. All audit calls are wrapped in `try/catch` — errors are silently ignored so they never break saves. Entity ID for dental chart entries is `"{patient_id}:{tooth_number}"`.
- **Audit log — entity types**: `AuditEntityType` = `'timeline_entry' | 'treatment_plan' | 'treatment_plan_item' | 'dental_chart' | 'patient' | 'document'`. Labels in `i18n.t.audit.entityTypes.*` (both DE/EN). `AuditEntry.svelte` uses i18n for entity labels — do NOT use hardcoded English strings.
...
- [x] Phase 8 / 8b (partial) — Appointment Scheduling + UX polish
